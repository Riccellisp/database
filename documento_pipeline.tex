...
\begin{center}
    \texttt{http://localhost:18000/api/v1}
\end{center}

\subsection{Endpoint: Liveness/Readiness Probe}
*   \textbf{Método}: \texttt{GET}
*   \textbf{Rota}: \texttt{/health}
*   \textbf{Descrição}: Verifica a integridade da API e a conectividade com o banco de dados MySQL.
...
async function carregarDadosGrafico(equipamentoId) {
  const url = `http://localhost:18000/api/v1/equipments/${equipamentoId}/telemetry?limit=48`;
  try {
